...
int BuildBuffer(int *cur_frame) {
	...
	for (i = 0; i < FRAMES_PER_BUF; i++) buf[i][0] = buf[i][1] = 0; 


	for (i = 0; i < FRAMES_PER_BUF; i++) {
		table_ind = tableIndexFromFreq((*cur_frame)++, freq);
...
int WriteBuffered() {
	...
	total_frames = time_ms * SAMPLE_RATE / 1000;
	buffer_writes = total_frames / FRAMES_PER_BUF; /* TODO: what to do with remainder? truncate? */
